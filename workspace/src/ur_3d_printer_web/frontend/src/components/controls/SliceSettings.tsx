...
import { DEFAULT_SLICE_SETTINGS } from '../../lib/constants';
    ...
    <Card title={t('slice.title')}>
      <div className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          ...
          <label className="block">
            <span className="text-xs text-gray-500 dark:text-gray-400">{t('slice.layerHeight')}</span>
            <input
              type="number"
              step="0.1"
              min="0.1"
              value={settings.layer_height}
              onChange={(e) => setSettings({ ...settings, layer_height: parseFloat(e.target.value) || 1.0 })}
              className="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm px-2 py-1.5"
            />
          </label>
          <label className="block">
            ...
            <input
              ...
              onChange={(e) => setSettings({ ...settings, nozzle_diameter: parseFloat(e.target.value) || 0.4 })}
              className="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm px-2 py-1.5"
            />
          </label>
          <label className="block">
